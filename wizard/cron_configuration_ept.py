...
class ShopifyCronConfigurationEpt(models.TransientModel):
    ...
    shopify_order_auto_export = fields.Boolean('Export Order', default=False,
                                               help="Check if you want to automatically Export Orders from Odoo to"
                                                    " Shopify.")
    shopify_export_order_interval_number = fields.Integer('Interval Number for Export Order', help="Repeat every x.")
    shopify_export_order_interval_type = fields.Selection([('minutes', 'Minutes'), ('hours', 'Hours'),
                                                           ('days', 'Days'), ('weeks', 'Weeks'),
                                                           ('months', 'Months')], 'Interval Unit for Export Order')
    shopify_export_order_next_execution = fields.Datetime('Next Execution for Export Order',
                                                          help='Next Execution for Export Order')
    shopify_export_order_user_id = fields.Many2one('res.users', string="User for Export Order",
    ...
    shopify_import_buy_with_prime_order_user_id = fields.Many2one('res.users',
                                                                  ...
                                                                  default=lambda self: self.env.user)
    # Auto cron for Import order return
    shopify_auto_import_order_return = fields.Boolean('Import Return/Refund from Shopify', default=False,
                                                      help="Check if you want to automatically Import Return/Refund"
                                                           " Orders from Shopify to Odoo.")
    shopify_import_order_return_interval_number = fields.Integer('Interval Number for Import Return/Refund Order',
                                                                help="Repeat every x.")
    shopify_import_order_return_interval_type = fields.Selection([('minutes', 'Minutes'), ('hours', 'Hours'),
                                                                  ('days', 'Days'), ('weeks', 'Weeks'),
                                                                  ('months', 'Months')],
                                                                 'Interval Unit for Import Return/Refund Order')
    shopify_import_order_return_next_execution = fields.Datetime(
        'Next Execution for Import Return/Refund Order',
        help='Next Execution for Import Return/Refund Order')
    shopify_import_order_return_user_id = fields.Many2one('res.users',
                                                          string="User for Import Return/Refund Order",
                                                          help='User for Import Return/Refund Order',
                                                          default=lambda self: self.env.user)
    ...
    @api.constrains("shopify_inventory_export_interval_number", "shopify_payout_import_interval_number",
                    ...
                    "shopify_product_import_interval_number","shopify_import_order_return_interval_number",
                     "shopify_export_order_interval_number")
    def check_interval_time(self):
        ...
        for record in self:
            ...
            if record.shopify_order_auto_import and record.shopify_import_order_interval_number <= 0:
                is_zero = True
            if record.shopify_order_auto_export and record.shopify_export_order_interval_number <= 0:
                is_zero = True
            ...
            if record.shopify_auto_import_product and record.shopify_product_import_interval_number <= 0:
                is_zero = True
            if record.shopify_auto_import_order_return and record.shopify_import_order_return_interval_number <= 0:
                is_zero = True
            if is_zero:
    ...
    def onchange_shopify_instance_id(self):
        ...
        self.update_import_return_order_cron_field(instance)
    ...
    def update_export_order_cron_field(self, instance: object):
        """
        Set export order cron fields value while open the wizard for cron configuration from the instance form view.

        :author: Karan Modasiya on Date 30-Mar-2026.
        """
        try:
            export_order_cron_exist = instance and self.env.ref(
                'shopify_ept.ir_cron_shopify_auto_export_order_instance_%d' % instance.id)
        except Exception:
            export_order_cron_exist = False
        if export_order_cron_exist:
            self.shopify_order_auto_export = export_order_cron_exist.active or False
            self.shopify_export_order_interval_number = export_order_cron_exist.interval_number or False
            self.shopify_export_order_interval_type = export_order_cron_exist.interval_type or False
            self.shopify_export_order_next_execution = export_order_cron_exist.nextcall or False
            self.shopify_export_order_user_id = export_order_cron_exist.user_id.id or False
    ...
    def update_import_return_order_cron_field(self, instance):
        """
        Set import return order cron fields value while open the wizard for cron configuration from the instance form view.
        """
        try:
            import_return_order_cron_exist = instance and self.env.ref(
                'shopify_ept.ir_cron_shopify_auto_import_order_return_instance_%d' % instance.id)
        except:
            import_return_order_cron_exist = False
        if import_return_order_cron_exist:
            self.shopify_auto_import_order_return = import_return_order_cron_exist.active or False
            self.shopify_import_order_return_interval_number = import_return_order_cron_exist.interval_number or False
            self.shopify_import_order_return_interval_type = import_return_order_cron_exist.interval_type or False
            self.shopify_import_order_return_next_execution = import_return_order_cron_exist.nextcall or False
            self.shopify_import_order_return_user_id = import_return_order_cron_exist.user_id.id or False
    ...
    def save(self):
        ...
        if instance:
            ...
            self.setup_shopify_export_order_cron(instance)
            ...
            self.setup_shopify_import_return_order_cron(instance)
    ...
    def setup_shopify_export_order_cron(self, instance: object):
        """
        Cron for auto Export Orders
        ...
        """
        try:
            cron_exist = self.env.ref('shopify_ept.ir_cron_shopify_auto_export_order_instance_%d' % instance.id)
        except Exception:
            cron_exist = False
        if self.shopify_order_auto_export:
            nextcall = datetime.now() + _intervalTypes[self.shopify_export_order_interval_type](
                self.shopify_export_order_interval_number)
            vals = self.prepare_val_for_cron(self.shopify_export_order_interval_number,
                                             self.shopify_export_order_interval_type,
                                             self.shopify_export_order_user_id)
            vals.update({'nextcall': self.shopify_export_order_next_execution or nextcall.strftime('%Y-%m-%d %H:%M:%S'),
                         'code': "model.export_order_cron_action(ctx={'shopify_instance_id':%d})" % instance.id,})
            if cron_exist:
                vals.update({'name': cron_exist.name})
                cron_exist.write(vals)
            else:
                core_cron = self.check_core_shopify_cron("shopify_ept.ir_cron_shopify_auto_export_order")

                name = instance.name + ' : ' + core_cron.name
                vals.update({'name': name})
                new_cron = core_cron.copy(default=vals)
                name = 'ir_cron_shopify_auto_export_order_instance_%d' % instance.id
                self.create_ir_module_data(name, new_cron)
        else:
            if cron_exist:
                cron_exist.write({'active': False})
    ...
    def setup_shopify_import_product_cron(self, instance):
        ...
        if self.shopify_auto_import_product:
            ...
            if cron_exist:
                cron_exist.write({'active': False})
        return True
    def setup_shopify_import_return_order_cron(self, instance):
        """
        Cron for auto Import Return Orders
        :param instance:
        :return:
        """
        try:
            ...
            cron_exist = False
        if self.shopify_auto_import_order_return:
            nextcall = datetime.now() + _intervalTypes[self.shopify_import_order_return_interval_type](
                self.shopify_import_order_return_interval_number)
            vals = self.prepare_val_for_cron(self.shopify_import_order_return_interval_number,
                                             self.shopify_import_order_return_interval_type,
                                             self.shopify_import_order_return_user_id)
            vals.update(
                {'nextcall': self.shopify_import_order_return_next_execution or nextcall.strftime('%Y-%m-%d %H:%M:%S'),
                 'code': "model.import_return_order_cron_action(ctx={'shopify_instance_id':%d})" % instance.id,
                 })
            if cron_exist:
                vals.update({'name': cron_exist.name})
                cron_exist.write(vals)
            else:
                core_cron = self.check_core_shopify_cron("shopify_ept.ir_cron_shopify_auto_import_order_return")
                name = instance.name + ' : ' + core_cron.name
                vals.update({'name': name})
                new_cron = core_cron.copy(default=vals)
                name = 'ir_cron_shopify_auto_import_order_return_instance_%d' % (instance.id)
                self.create_ir_module_data(name, new_cron)
        else:
            if cron_exist:
                cron_exist.write({'active': False})
