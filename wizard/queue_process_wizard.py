...
class ShopifyQueueProcessEpt(models.TransientModel):
    ...
    def manual_queue_process(self):
        ...
        """
        queue_process = self.env.context.get('queue_process')
        ...
        if queue_process == "process_export_metafields_queue_manually":
            self.sudo().process_export_metafield_queue_manually()
    ...
    def process_product_queue_manually(self):
        ...
        product_queue_ids = product_queue_data_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != 'done')
        if model == 'shopify.product.data.queue.line.ept':
            queue_line_active_ids.extend(self.env.context.get('active_ids'))
    ...
    def process_customer_queue_manually(self):
        ...
        queue_line_active_ids = []
        customer_queue_ids = customer_queue_data_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != 'done')
    ...
    def process_order_queue_manually(self):
        ...
        queue_line_active_ids = []
        order_queue_ids = order_queue_data_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != 'done')
    ...
    def process_export_stock_queue_manually(self):
        ...
        queue_line_active_ids = []
        export_stock_queue_ids = export_stock_queue_data_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != 'done')
    ...
    def process_export_metafield_queue_manually(self):
        """
            This method used to process the export metafields queue manually.
            @author: SejalK @Emipro Technologies Pvt.Ltd on date 13-March-2026.
            Task Id : 47836,47838
        """
        model = self.env.context.get('active_model')
        export_metafield_queue_data_obj = self.env[model]
        queue_line_active_ids = []
        export_metafield_queue_ids = export_metafield_queue_data_obj.browse(self.env.context.get('active_ids')).filtered(
            lambda x: x.state != 'done')
        if model == 'shopify.export.metafields.queue.line.ept':
            queue_line_active_ids.extend(self.env.context.get('active_ids'))
            export_metafield_queue_ids = export_metafield_queue_ids.mapped('export_metafields_queue_id').filtered(
                lambda queue: queue.state != 'done')
        for export_metafield_queue_id in export_metafield_queue_ids:
            export_metafield_queue_line_ids = export_metafield_queue_id.export_metafields_queue_line_ids.filtered(
                lambda x: x.state in ['draft', 'failed'])
            if queue_line_active_ids:
                export_metafield_queue_line_ids = export_metafield_queue_line_ids.filtered(
                    lambda line: line.id in queue_line_active_ids)
            if export_metafield_queue_line_ids:
                export_metafield_queue_line_ids.process_export_metafields_queue_data()
        return True
    ...
    def set_to_completed_queue(self):
        ...
        if queue_process == "set_to_completed_export_metafield_queue":
            self.set_to_completed_export_metafield_queue_manually()
    ...
    def set_to_completed_export_stock_queue_manually(self):
        ...
        export_stock_queue_ids = self.env.context.get('active_ids')
    ...
    def set_to_completed_order_queue_manually(self):
        ...
        """
        order_queue_ids = self.env.context.get('active_ids')
    ...
    def set_to_completed_product_queue_manually(self):
        """This method used to set product queue as completed. You can call the method from here :
            ...
            @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 25/12/2019.
        """
        product_queue_ids = self.env.context.get('active_ids')
    ...
    def set_to_completed_customer_queue_manually(self):
        """This method used to set customer queue as completed. You can call the method from here :
            ...
            @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 25/12/2019.
        """
        customer_queue_ids = self.env.context.get('active_ids')
    ...
    def set_to_completed_export_metafield_queue_manually(self):
        """This method used to set export metafield queue as completed. You can call the method from here :
            Shopify => Processes => Queues Logs => Export Metafield Queues => SET TO COMPLETED. """
        export_metafield_queue_ids = self.env.context.get('active_ids')
        export_metafield_queue_ids = self.env['shopify.export.metafields.queue.ept'].browse(export_metafield_queue_ids)
        for export_metafield_queue_id in export_metafield_queue_ids:
            queue_lines = export_metafield_queue_id.export_metafields_queue_line_ids.filtered(
                lambda line: line.state in ['draft', 'failed'])
            queue_lines.write({'state': 'cancel'})
        return True
    ...
    def instance_active_archive(self):
        ...
        :return:
        """
        instances = self.env['shopify.instance.ept'].browse(self.env.context.get('active_ids'))
