...
class ShopifyProcessImportExport(models.TransientModel):
    ...
    shopify_operation = fields.Selection(
        [("sync_product", "Import Products"),
         ...
         ("import_order_returns", "Import Returns/ Refunds"),
         ...
         ('import_metafields', "Import Metafields"),
         ('import_customer_segment', "Import Customer Segment"),
         ],
    ...
    fetch_scope = fields.Selection([
        ('PRODUCT', 'PRODUCT'),
        ('PRODUCTVARIANT', 'PRODUCT VARIANT'),
        ('ORDER', 'ORDER'),
        ('CUSTOMER', 'CUSTOMER'),
          ], string='Fetch Scope', help='Select metafield scope')

    metafield_key = fields.Char(string='Metafield Key', help='Fetch particular meta fields')
    ...
    @api.onchange('orders_to_date')
    def onchange_orders_to_date(self):
        if self.orders_to_date and self.orders_to_date > fields.Datetime.now() + timedelta(minutes=1):
            raise ValidationError("The 'To Date' cannot be set to a future date and time.")

    @api.onchange("payout_end_date")
    def onchange_payout_end_date(self):
        if self.payout_end_date and self.payout_end_date > fields.Date.today():
            raise ValidationError("The 'End Date' cannot be set to a future Date.")
    ...
    def shopify_execute(self):
        ...
        if self.shopify_operation == "sync_product":
            product_queue_ids = product_data_queue_obj.with_context(queue_created_by="manual").shopify_create_product_data_queue(
            ...
            customer_queues = self.export_customer()
            if customer_queues:
                queue_ids = customer_queues
                action_name = "shopify_ept.action_shopify_synced_customer_data"
                form_view_name = "shopify_ept.shopify_synced_customer_data_form_view_ept"
            ...
            order_queues = order_date_queue_obj.with_context(queue_created_by="manual").shopify_create_order_data_queues(instance, self.orders_from_date,
                                                                                 self.orders_to_date,
            ...
            order_queues = order_date_queue_obj.with_context(queue_created_by="manual").shopify_create_order_data_queues(instance,
        ...
        elif self.shopify_operation == "import_order_returns":
            return self.import_shopify_order_returns()

        elif self.shopify_operation == "import_orders_by_remote_ids":
            order_queues = order_date_queue_obj.with_context(queue_created_by="manual").import_order_process_by_remote_ids(instance, self.shopify_order_ids)
            if order_queues:
                queue_ids = order_queues
                action_name = "shopify_ept.action_shopify_order_data_queue_ept"
                form_view_name = "shopify_ept.view_shopify_order_data_queue_ept_form"
        ...
        elif self.shopify_operation == "import_metafields":
            if not instance.enable_metafield_sync:
                raise UserError(_("Metafield sync is not enabled for this Shopify instance. Please enable 'Metafield Synchronization' in the instance configuration to proceed."))
            message, create_ids = self.env['shopify.metafield.config.ept'].import_shopify_metafield_configs(instance,self.fetch_scope,self.metafield_key)
            if message:
                raise UserError(message)   
            
            if create_ids:  
                action_name = self.env.ref("shopify_ept.action_shopify_metafield_config").sudo().read()[0]             
                action_name['domain']=[("id", "in", create_ids)]
            
                return action_name
        elif self.shopify_operation == "import_customer_segment":
            if not instance.use_graphql_api:
                raise UserError(
                    _("Customer Segment import is only available for instances using GraphQL API. Please enable 'Use GraphQL API' in the instance configuration to proceed."))
            self.env['shopify.customer.segment.ept'].import_customer_segment(instance)
            return self.env.ref('shopify_ept.action_shopify_customer_segment').sudo().read()[0]
    ...
    def import_shopify_order_returns(self):
        """Import Shopify return records for selected date range."""
        instance = self.shopify_instance_id
        if not self.orders_from_date or not self.orders_to_date:
            raise UserError(_("Please provide both From Date and To Date to import order returns."))

        now_dt = fields.Datetime.now()
        if self.orders_from_date > self.orders_to_date:
            raise UserError(_("The 'From Date' must be earlier than or equal to 'To Date'."))
        if self.orders_to_date > now_dt or self.orders_from_date > now_dt:
            raise UserError(_("From Date and To Date cannot be in the future."))
        if not instance.use_graphql_api:
            raise UserError(_("Import Order Returns is supported only when 'Use GraphQL API' is enabled."))

        company = instance.shopify_company_id
        sale_order_model = self.env["sale.order"].with_company(company).with_context(
            allowed_company_ids=[company.id],
        )
        sale_order_model.import_shopify_order_returns_ept(instance, self.orders_from_date, self.orders_to_date)
    ...
    def manual_export_product_to_shopify(self):
        ...
        shopify_products = self.env.context.get("active_ids", [])
        ...
        for instance in shopify_instances:
            ...
            if shopify_templates:
                ...
                if instance.enable_metafield_sync:
                    ...
                    self.env["shopify.export.metafields.queue.ept"].prepare_export_metafield_queue_line(instance, shopify_templates)
        end = time.time()
        ...
        for instance in shopify_instances:
            ...
            if shopify_templates:
                if instance.use_graphql_api:
                    shopify_product_obj.update_products_in_shopify_via_graphql(instance, shopify_templates,
                                                                               self.shopify_is_set_price,
                                                                               self.shopify_is_set_image,
                                                                               self.shopify_is_publish,
    ...
    def sync_product_layer_with_shopify(self):
        """Fetches all active/draft products from the Shopify store and removes layer records
        for products that no longer exist there."""
        shopify_product_obj = self.env['shopify.product.product.ept']

        if not self.shopify_instance_id:
            return {'type': 'ir.actions.act_window_close'}
        instance = self.shopify_instance_id
        all_exported = self.env['shopify.product.template.ept'].search(
            [('exported_in_shopify', '=', True), ('shopify_instance_id', '=', instance.id)])
        before_count = len(all_exported)
        instance.connect_in_shopify()
        valid_templates = shopify_product_obj.check_available_products_in_shopify(instance)
        total_removed = before_count - len(valid_templates)
        ...
        return {
            ...
            'params': {
                ...
                'message': _('%d stale product layer record(s) removed.') % total_removed,
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def sync_shopify_customers(self):
        ...
        instance = self.shopify_instance_id
        instance.connect_in_shopify()
        last_import = instance.shopify_last_date_customer_import
        if instance.use_graphql_api:
            # GraphQL path: returns tuple (records, has_next_page, end_cursor)
            customer_data = self.fetch_graphql_customer_data(instance, last_import)
            customer_ids = customer_data[0] if customer_data else []
            if customer_ids:
                customer_queues_ids = self.create_customer_data_queues(customer_ids)
                customer_queues_ids += self.fetch_graphql_all_customer_data(
                    customer_data, last_import)
        else:
            # REST path: returns flat list
            instance.connect_in_shopify()
            if not last_import:
                customer_ids = shopify.Customer().find(limit=250)
            else:
                customer_ids = shopify.Customer().find(
                    updated_at_min=last_import, limit=250
                )
            if customer_ids:
                customer_queues_ids = self.create_customer_data_queues(customer_ids)
        ...
        if not customer_ids:
            ...
            queue_cron = self.sudo().env.ref("shopify_ept.process_shopify_customer_queue")
            if not queue_cron.active:
                _logger.info("Active the Customer data process queue cron job")
                queue_cron.write(
    ...
    def export_customer(self):
        """
        This method used to export the customers data from Odoo to Shopify.
        :return: customer queue ids
        ...
        partners_to_export = self._get_exportable_partners(instance)
        if partners_to_export:
            customer_queues_ids = self.create_export_customer_data_queues(partners_to_export)
        if not partners_to_export:
            _logger.info("No customers found for export to Shopify.")
        else:
            queue_cron = self.sudo().env.ref("shopify_ept.process_shopify_customer_queue")
            if not queue_cron.active:
                _logger.info("Active the Customer data process queue cron job")
                queue_cron.write({'active': True, 'nextcall': datetime.now() + timedelta(seconds=120)})
        return customer_queues_ids
    ...
    def create_customer_data_queues(self, customer_data):
        ...
        if len(customer_data) > 0:
            for customer_id_chunk in split_every(125, customer_data):
                ...
                message = "Customer Queue created %s" % ', '.join(customer_queue.mapped('name'))
    ...
    def create_export_customer_data_queues(self, partners: list[object]):
        """
        This method is used to create customer data queue for export customers from Odoo to Shopify.
        :param partners: List of partner record which are used to export customers in shopify
        :return: customer queue list
        ...
        if len(partners) > 0:
            for partner_id_chunk in split_every(50, partners):
                customer_queue = False
                updated_existing_queue = False
                existing_queue = customer_data_queue_obj.search(
                    [('shopify_instance_id', '=', self.shopify_instance_id.id),
                     ('record_created_from', '=', 'export_process'), ('state', '=', 'draft'),
                     ('is_export_queue', '=', True)])
                if existing_queue:
                    queues = existing_queue.filtered(lambda queue: len(queue.synced_customer_queue_line_ids) < 50)
                    if queues:
                        customer_queue = queues[0]
                        updated_existing_queue = True
                if not customer_queue:
                    customer_queue = customer_data_queue_obj.create_customer_queue(self.shopify_instance_id,
                                                                                   "export_process")
                    customer_queue.write({"is_export_queue": True})
                customer_data_queue_line_obj.shopify_create_multi_queue_export_partner(customer_queue, partner_id_chunk)

                if updated_existing_queue:
                    message = "Customer Queue updated %s" % ', '.join(customer_queue.mapped('name'))
                else:
                    message = "Customer Queue created %s" % ', '.join(customer_queue.mapped('name'))
                bus_bus_obj._sendone(self.env.user.partner_id, "simple_notification",
                                     {"title": "Shopify Notification", "message": message, "sticky": False,
    ...
    def shopify_list_all_customer(self, result):
        ...
        while result:
            ...
            link = shopify.ShopifyResource.connection.response.headers.get(
                'link') or shopify.ShopifyResource.connection.response.headers.get('Link')
    ...
    def import_return_order_cron_action(self, ctx=False):
        """This method is used to import order returns from the auto-import cron job.
        """
        if not isinstance(ctx, dict):
            return True
        instance_id = ctx.get('shopify_instance_id')
        instance = self.env['shopify.instance.ept'].browse(instance_id)
        from_date = instance.last_return_sync_date
        to_date = datetime.now()
        if not from_date:
            from_date = to_date - timedelta(3)
        self.env["sale.order"].import_shopify_order_returns_ept(instance, from_date, to_date)
        return True
    ...
    def search_product_for_fixed_stock_export(self, instance):
        ...
        @author: Yagnik Joshi @Emipro Technologies Pvt. Ltd on date 12 December 2023 .
        Task_id:4323
        """
        result = []
    ...
    def shopify_selective_product_stock_export(self):
        ...
        """
        shopify_product_obj = self.env['shopify.product.product.ept']
        shopify_template_ids = self.env.context.get('active_ids')
            ...
            if odoo_product_ids:
                ...
                if export_stock_queue_id:
                    ...
                    _logger.info(
                        ...
                        export_stock_data_queue.mapped('name'), instance.name)
    ...
    def onchange_shopify_instance_id(self):
        ...
        if instance:
            ...
            if self.shopify_operation == "import_unshipped_orders":
                ...
                self.orders_from_date = instance.last_return_sync_date or False
            elif self.shopify_operation == "import_buy_with_prime_orders":
    ...
    def create_or_update_shopify_template_from_csv(self, instance, record, shopify_template_id, sequence):
        ...
        product_tmpl_id = record.get("PRODUCT_TEMPLATE_ID")
        category_id = record.get("CATEGORY_ID")

        product_tmpl_id = int(product_tmpl_id) if product_tmpl_id and product_tmpl_id not in ("False", "false",
                                                                                              False) else False
        category_id = int(category_id) if category_id and category_id not in ("False", "false", False) else False

        shopify_template = shopify_product_template.search(
            [("shopify_instance_id", "=", instance.id),
             ("product_tmpl_id", "=", int(record["PRODUCT_TEMPLATE_ID"])),
             ("shopify_product_ids.default_code", "=", record["shopify_product_default_code"]),
             ], limit=1)

        shopify_product_template_vals = {"product_tmpl_id": product_tmpl_id,
                                         "shopify_instance_id": instance.id,
                                         "shopify_product_category": category_id,
        ...
        for sheet in sheets.sheets():
            for row_no in range(sheet.nrows):
                ...
                for k, v in header.items():
                    cell_value = sheet.row(row_no)[v].value
                    if isinstance(cell_value, float):
                        if cell_value.is_integer():
                            cell_value = int(cell_value)
                    row.update({k: cell_value})
    ...
    # def import_shopify_metafields(self):
    #     """ This method is used to call the child method to import the Shopify metafields from Shopify to Odoo.           
    #         Task_id: 168214 - Import metafields process
    #     """
    #     shopify_metafield_obj = self.env["shopify.metafield.config.ept"]
    #     shopify_metafields = shopify_metafield_obj.import_shopify_metafield_configs(self.shopify_instance_id,self.fetch_scope,self.metafield_key)
    ...
    def fetch_graphql_customer_data(self, instance, last_date_customer_import=False):
        client = instance.get_graphql_client()
        if last_date_customer_import:
            last_date_customer_import = pytz.utc.localize(last_date_customer_import).astimezone(
                pytz.timezone(instance.shopify_store_time_zone or
                              'UTC')).strftime('%Y-%m-%dT%H:%M:%S%z')
        query_filter = last_date_customer_import and (f"updated_at:>='{last_date_customer_import}'") or ""
        customers_list = shopify_graphql.CustomerQueryHelper(client, is_enable_metafield_sync=instance.enable_metafield_sync).get_shopify_customers(query_filter=query_filter,
                                                                                           no_pagination=True)
        return customers_list

    def fetch_graphql_all_customer_data(self, customers, last_date_customer_import=False):
        customer_queue_list = []
        instance = self.shopify_instance_id
        client = instance.get_graphql_client()
        if last_date_customer_import:
            last_date_customer_import = pytz.utc.localize(last_date_customer_import).astimezone(
                pytz.timezone(instance.shopify_store_time_zone or
                              'UTC')).strftime('%Y-%m-%dT%H:%M:%S%z')
        query_filter = last_date_customer_import and (f"updated_at:>='{last_date_customer_import}'") or ""
        while len(customers) == 3 and customers[1]:
            extra_connection_args = f'after:"{customers[2]}"'
            customers = shopify_graphql.CustomerQueryHelper(client, is_enable_metafield_sync=instance.enable_metafield_sync).get_shopify_customers(query_filter=query_filter,
                                                                                          no_pagination=True,
                                                                                          extra_connection_args=extra_connection_args)
            customer_queue_list += self.create_customer_data_queues(customers[0])
        return customer_queue_list

    def _get_exportable_partners(self, instance: object):
        """
        This method is used to get the exportable partners for the Shopify instance.
        ...
        partners = res_partner.search([('parent_id', '=', False), ('email', '!=', False),
                                       ('id', 'not in', shopify_partners.ids)])
        return partners
