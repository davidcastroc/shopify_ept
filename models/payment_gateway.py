...
class ShopifyPaymentGateway(models.Model):
    ...
    def shopify_search_create_gateway_workflow_market(self, instance, order_data_queue_line,
                                                      order_response, gateway, market=False):
        """
        This method is similar to `shopify_search_create_gateway_workflow` but
        searches the workflow configuration in market-specific model
        `market.auto.workflow.configuration.ept`.

        If market is not provided and cannot be resolved from order data,
        method safely falls back to instance-level workflow configuration.

        :param instance: Shopify instance record
        :param order_data_queue_line: Record of order data queue line
        :param order_response: Shopify order response
        :param gateway: payment gateway name
        :param market: optional shopify.market.ept record
        :return: gateway, workflow, payment_term
        """
        common_log_line_obj = self.env["common.log.lines.ept"]
        auto_workflow_id = False
        ...
        order_status = 'unshipped'
        if order_response.get('fulfillment_status') and order_response.get('fulfillment_status') != 'unfulfilled':
            order_status = order_response.get('fulfillment_status')

        if not market:
            market_obj = self.env["shopify.market.ept"]
            country_code = (order_response.get("billing_address") or {}).get("country_code", "")
            currency_code = order_response.get("presentment_currency") or order_response.get("currency") or ""
            market = market_obj.find_market_for_order_ept(
                instance,
        ...
        if not market:
            return self.shopify_search_create_gateway_workflow(
                instance,
        ...
        workflow_config = self.env['market.auto.workflow.configuration.ept'].search(
        ...
        if not workflow_config:
            message = "- Market Automatic order process workflow configuration not found for this order " \
                      "%s. \n - System tries to find the workflow based on Market + Payment " \
                      "Gateway(such as Manual,Credit Card, Paypal etc.) and Financial Status(such as Paid," \
                      "Pending,Authorised etc.).\n - In this order Market is %s, Payment Gateway is %s , " \
                      "Financial Status is %s and order status is %s." \
                      " \n - You can configure the Automatic order process workflow under market " \
                      "configuration in Shopify > Configuration > Markets." % (
