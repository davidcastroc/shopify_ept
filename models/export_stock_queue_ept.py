...
class ShopifyExportStockQueueEpt(models.Model):
    ...
    def create_export_stock_queue(self, instance, export_stock_data):
        ...
        # Delete empty queues (all products in that batch already had draft lines on older queues).
        empty_queues = created_queues.filtered(lambda q: not q.export_stock_queue_line_ids)
        if empty_queues:
            _logger.info("Deleting %d empty export stock queue(s): %s",
                         len(empty_queues), ', '.join(empty_queues.mapped('name')))
            created_queues -= empty_queues
            empty_queues.unlink()

        # Return all non-empty queues created in this run
