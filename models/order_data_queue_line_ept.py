...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    def create_order_queue_line(self, order_dict, instance, order_data, customer_name, customer_email, order_queue_id):
        ...
        result = self.get_shopify_order_transanctions(instance, order_dict) or []
    ...
    def get_shopify_order_transanctions(self, instance, order_dict):
        """
        This method is used to get the transaction data from shopify.
        """
        result = []
        transactions = []
        ...
        if len(order_dict.get('payment_gateway_names')) >= 1:
            if instance.use_graphql_api :
                result = order_dict.get('transaction', [])
                if not result:
                    client = instance.get_graphql_client()
                    transactions = shopify_graphql.OrderQueryHelper(client,
                                                                    order_visible_currency=not in_shop_currency,is_enable_metafield_sync=instance.enable_metafield_sync).get_order(
                        [order_dict.get('id')], specific_fields_key='transactions')
                    result = transactions and transactions[0].get('transaction', [])
            else:
    ...
    def create_order_data_queue_line(self, orders_data, instance, queue_type, created_by="import"):
        ...
        """
        if instance.use_graphql_api:
            return self.create_order_data_queue_line_by_gql_api(orders_data, instance, queue_type,
        ...
        order_queue_list = []
        fulfillment_data = []
        is_new_order = bool(self.env.context.get('is_new_order'))
    ...
    def auto_import_order_queue_data(self):
        ...
        """
        params = ('draft', 'False')
        self.env.cr.execute(select_query, params)
    ...
    def create_order_data_queue_line_by_gql_api(self, orders_data, instance, queue_type, created_by="import"):
        """
        This method used to create order data queue lines from Graphql API. It creates new queue after 50 order queue lines.
        """
        count = 0
        order_queue = False
        need_to_create_queue = True
        ...
        for order in orders_data:
            ...
            if instance.import_buy_with_prime_shopify_order:
                queue_type_is_buy_with_prime = any(
                    buy_with_prime_tag.name in data.get("tags") for buy_with_prime_tag in
                    instance.buy_with_prime_tag_ids)
            data.update({"buy_with_prime": queue_type_is_buy_with_prime})
            customer_name, customer_email = self.get_customer_name_and_email(order)
            self.create_order_queue_line(order, instance, data, customer_name, customer_email, order_queue)
            count += 1
            if count == 50:
