...
from ..shopify_graphql.queries.translation import TranslationQueryHelper, get_digest

utc = pytz.utc
_logger = logging.getLogger("Shopify Template")

# Maximum number of products per batched translation API call.
# Keeps GraphQL query complexity well under Shopify's 1 000-point limit.
...
class ShopifyProductTemplateEpt(models.Model):
    ...
    def find_template_attribute_values(self, template_options, product_template_id, variant):
        ...
        product_template = self.env["product.template"].search([('id', '=', product_template_id)], limit=1)
        odoo_attribute_lines = product_template.attribute_line_ids.filtered(
            lambda x: x.attribute_id.create_variant == "always")
        for attribute in template_options:
            product_attribute = odoo_attribute_lines.filtered(
                lambda x: x.display_name == attribute.get("name")).attribute_id
    ...
    def shopify_create_simple_product(self, product_name, variant_data, description, attribute_line_data=[]):
        ...
        if not variant_data.get('requires_shipping'):
    ...
    def import_product_for_order(self, template_id, order_queue_line, model_name, instance):
        ...
        try:
            if instance.use_graphql_api:
                ...
                result = shopify_graphql.ProductQueryHelper(client).get_product([template_id], convert_to_rest=True,
                                                                                gql_object_form=True)
            else:
    ...
    def prepare_variant_vals(self, instance, variant_data):
        ...
        variant_vals = {"shopify_instance_id": instance.id,
                        ...
                        "shopify_hsn_code": variant_data.get("harmonizedSystemCode"),
                        "active": True}
    ...
    def shopify_sync_products(self, product_data_line_id, shopify_tmpl_id, instance,
        ...
        if instance.auto_create_product_category:
            product_category = self.get_product_category(template_data.get("product_type"))
        elif not instance.auto_create_product_category and instance.shopify_instance_product_category:
            product_category = instance.shopify_instance_product_category if instance.shopify_instance_product_category else False
        ...
        if shopify_template:
            shopify_template = self.sync_product_with_existing_template(shopify_template, skip_existing_product,
                                                                        ...
                                                                        order_data_line_id)
            if shopify_template:
                self.sync_product_tags(shopify_template, instance)
        ...
        if instance.enable_metafield_sync:
            if instance.use_graphql_api:
                self.set_metafield_values_in_product_template(template_data, instance, shopify_template)
            else:
                # get template metafield values using rest api and set in product template.
                owner_id = "gid://shopify/Product/" + str(template_data.get("id"))
                template_metadeta=self.env['shopify.metafield.config.ept'].get_resource_metafield_by_owner(instance, owner_id)
                self.set_metafield_values_in_product_template(template_metadeta, instance, shopify_template)

        # NOTE: Translations are NOT imported here per-product.
        # The caller (process_product_queue_line_data) collects all synced templates
        # and calls batch_import_product_translations() once for the entire batch.

        return shopify_template

    def sync_product_tags(self, shopify_template, instance):
        """
        Syncs tags from Shopify to Odoo Product Template.
        Only adds tags that are not already present on the template.
        """
        if not instance.auto_sync_product_tag or not shopify_template.tag_ids:
            return True

        # 1. Identify the Odoo Template record
        product_tmpl = False
        first_variant = shopify_template.shopify_product_ids[:1]
        if first_variant and first_variant.product_id:
            product_tmpl = first_variant.product_id.product_tmpl_id

        if not product_tmpl:
            return True

        # 2. Get currently linked tags to avoid redundant writes
        existing_tag_ids = product_tmpl.product_tag_ids.ids
        tag_commands = []
        processed_names = set()

        for tag in shopify_template.tag_ids:
            raw_name = tag.name or ""
            clean_name = raw_name.strip()

            # Avoid processing empty strings or duplicate names in the same loop
            if not clean_name or clean_name.lower() in processed_names:
                continue

            # 3. Find or Create the tag in the Odoo Master Table (product.tag)
            product_tag = self.env['product.tag'].search([('name', '=ilike', clean_name)], limit=1)
            if not product_tag:
                product_tag = self.env['product.tag'].create({'name': clean_name})

            # 4. CRITICAL CHECK: Only add if tag is NOT already on this specific product
            if product_tag.id not in existing_tag_ids:
                tag_commands.append((4, product_tag.id))

            processed_names.add(clean_name.lower())

        # 5. Only perform a write if there are actually NEW tags to add
        if tag_commands:
            product_tmpl.write({'product_tag_ids': tag_commands})

        return True

    def update_weight_product_variants(self, instance, shopify_template, variant_data):
        if not instance.shopify_sync_product_weight:
            return True
    ...
    def sync_product_with_existing_template(self, shopify_template, skip_existing_product, template_data, instance,
        ...
        variant_ids, need_to_archive = self.sync_variant_data_with_existing_template(instance, variant_data,
                                                                                     ...
                                                                                     order_data_line_id,
                                                                                     model_name)
        if len(variant_ids) == len(variant_data):
            shopify_response_variant_ids = {int(v.get("id")) for v in variant_data if v.get("id")}
            products_to_archive = shopify_template.shopify_product_ids.filtered(
                lambda x: x.variant_id and int(x.variant_id) not in shopify_response_variant_ids)
            if products_to_archive:
                _logger.info("Archiving %d Shopify variant(s) deleted from Shopify store for template: %s",
                             len(products_to_archive), shopify_template.name)
                products_to_archive.write({"active": False})
        elif need_to_archive:
            products_to_archive = shopify_template.shopify_product_ids.filtered(
    ...
    def sync_variant_data_with_existing_template(self, instance, variant_data, template_data, shopify_template,
        ...
        for variant in variant_data:
                    ...
                    if not shopify_product:
                        if instance.auto_import_product:
                            if odoo_template.attribute_line_ids:
                                ...
                                attribute_line_data = self.prepare_attribute_line_data_for_variant(shopify_attributes,
                                                                                                   variant)
            ...
            # ====================================================================================
            # sejalk : 020326 Create new product or template then find metafields configuration and 
            #   set metafield values in the product template & product variant.         
            # if shopify instance have metafield configuration then only set metafield values in product template and variant.
            if instance.enable_metafield_sync:
                if instance.use_graphql_api:
                    self.set_metafield_values_in_product_variant(variant,instance, shopify_product)
                else:
                    # get template metafield values using rest api and set in product template.
                    owner_id = "gid://shopify/ProductVariant/" + str(variant.get("id"))
                    variant_metadeta=self.env['shopify.metafield.config.ept'].get_resource_metafield_by_owner(instance, owner_id)
                    self.set_metafield_values_in_product_variant(variant_metadeta, instance, shopify_product)
        ...
        for variant in variant_data:
            ...
            if shopify_product:
                ...
                if odoo_template and odoo_template.attribute_line_ids:
                    ...
                    # Setting the product category in the product template when new product is created.
                    new_odoo_template = odoo_product.product_tmpl_id
                    new_odoo_template.write({'categ_id': product_category and product_category.id or False})
                    need_to_update_template = False
                if isinstance(shopify_product, str):
                    message = shopify_product
                    self.create_log_line_for_queue_line(instance, message, model_name, product_data_line_id,
            ...
            if instance.enable_metafield_sync:
                if instance.use_graphql_api:
                    ...
                    # get variant metafield values using rest api and set in product variant.
                    # ownerid="gid://shopify/ProductVariant/10282266722603",
                    owner_id = "gid://shopify/ProductVariant/" + str(variant.get("id"))
                    variant_metadeta = self.env['shopify.metafield.config.ept'].get_resource_metafield_by_owner(instance, owner_id)
                    self.set_metafield_values_in_product_variant(variant_metadeta, instance, shopify_product)
            ...
            instance.shopify_pricelist_id.set_product_price_ept(shopify_product.product_id.id, variant.get("price"))
            if instance.shopify_compare_pricelist_id:
                instance.shopify_compare_pricelist_id.set_product_price_ept(shopify_product.product_id.id,
    ...
    def check_for_new_variant(self, odoo_template, shopify_attributes, variant_data, shopify_template, variant_vals):
        ...
        if not attribute_value_domain:
            for shopify_attribute in shopify_attributes:
                ...
                attribute_id = odoo_attribute_lines.filtered(
                    lambda x: x.display_name.lower() == shopify_attribute.get("name").lower()).attribute_id.id
                value_id = \
        ...
        barcode = variant_data.get("barcode")
        if barcode and odoo_product and not odoo_product.barcode:
            odoo_product.barcode = barcode
    ...
    def shopify_sync_product_images(self, template_data):
        ...
        # We have handel the case like A-odoo product with shopify a1 and a2 product link so while
        # creating common product record, it reflects in both shopify product. Image should be display shopify product wise.
        # other_shopify_products = self.search([('id','!=',self.id),('product_tmpl_id','=',self.product_tmpl_id.id)])
        # if other_shopify_products and shopify_product_images:
        #     remove_images = self.env['shopify.product.image.ept']
        #     image_ids = shopify_product_images.shopify_image_id.ids
        #     for shopify_product_temp in other_shopify_products:
        #         s_images = shopify_product_image_obj.search([("shopify_template_id","=", shopify_product_temp.id),('shopify_image_id','in', )])
        #     if remove_images:
        #         remove_images.unlink()
                ...
                if response.status_code == 200:
                    ...
                    if key in existing_common_template_images.keys():
                        ...
                        if not self.product_tmpl_id.image_1920:
                            ...
                            common_product_image = self.product_tmpl_id.ept_image_ids.filtered(lambda x: x.image == self.product_tmpl_id.image_1920)
                        else:
    ...
    def shopify_prepare_template_dic(self, template_data, instance, product_category):
        ...
        for new_template in template_data.get('variants'):
            if not new_template.get('requires_shipping'):
    ...
    def is_product_importable(self, template_data, instance, odoo_product, shopify_product):
        ...
        if not odoo_product and not shopify_product and instance.shopify_sync_product_with in ["barcode",
                                                                                               "sku_or_barcode"] and not instance.auto_import_product:
        ...
        if self.shopify_tmpl_id:
            try:
                ...
                if new_product:
                    ...
                    if self.env.context.get("publish") == "shopify_unpublish":
                        new_product.published_scope = "null"
                        new_product.published_at = None
                    elif self.env.context.get("publish") == "shopify_publish_global":
    ...
    def batch_import_product_translations(self, instance, template_product_pairs):
        """
        Import translations for *all* products in one (or a few) batched GraphQL query.
        :param instance: shopify.instance.ept record
        :param template_product_pairs: list of (shopify_template_record, shopify_product_id_str)
        """
        if not instance.is_translations_needed() or not template_product_pairs:
            return

        client = instance.get_graphql_client()
        secondary_mappings = instance.get_secondary_lang_mappings()
        locale_to_lang = {m.shopify_locale: m.odoo_lang_id.code for m in instance.shopify_lang_mapping_ids}
        set_sales_desc = self.env["ir.config_parameter"].sudo().get_param("shopify_ept.set_sales_description")

        valid_pairs = [(t, pid) for t, pid in template_product_pairs if pid]
        if not valid_pairs:
            return

        _logger.info("batch_import_product_translations: %d product(s), chunk_size=%d.",
                     len(valid_pairs), TRANSLATION_CHUNK_SIZE)

        for chunk_start in range(0, len(valid_pairs), TRANSLATION_CHUNK_SIZE):
            chunk = valid_pairs[chunk_start: chunk_start + TRANSLATION_CHUNK_SIZE]
            helper = TranslationQueryHelper(client)
            gid_to_template = {}

            for shopify_template, shopify_product_id in chunk:
                gid = "gid://shopify/Product/%s" % shopify_product_id
                gid_to_template[gid] = shopify_template
                for mapping in secondary_mappings:
                    helper.add_target_to_pull(gid, mapping.shopify_locale)

            results = helper.pull_translations()
            _logger.info("batch_import_product_translations: chunk [%d-%d] → %d result(s).",
                         chunk_start, chunk_start + len(chunk) - 1, len(results))

            for result in results:
                locale = result.get("locale")
                gid = result.get("gid", "")
                translations = result.get("translations", {})
                odoo_lang_code = locale_to_lang.get(locale)
                shopify_template = gid_to_template.get(gid)

                if not odoo_lang_code or not shopify_template:
                    continue

                name_val = translations.get("title") or ""
                desc_val = translations.get("body_html") or ""
                if not name_val and not desc_val:
                    continue

                shopify_vals = {}
                if name_val:
                    shopify_vals["name"] = name_val
                if desc_val:
                    shopify_vals["description"] = desc_val
                if shopify_vals:
                    shopify_template.with_context(lang=odoo_lang_code).write(shopify_vals)

                if shopify_template.product_tmpl_id and instance.import_translation_in_odoo_product:
                    odoo_vals = {}
                    if name_val:
                        odoo_vals["name"] = name_val
                    if desc_val and set_sales_desc:
                        odoo_vals["description_sale"] = desc_val
                    if odoo_vals:
                        shopify_template.product_tmpl_id.with_context(lang=odoo_lang_code).write(odoo_vals)

    def batch_export_product_translations(self, instance, template_gid_pairs):
        """
        Export translations for *all* products in one (or a few) batched GraphQL mutation.
        :param instance: shopify.instance.ept record
        :param template_gid_pairs: list of (shopify_template_record, product_gid_str)
        """
        if not instance.is_translations_needed() or not template_gid_pairs:
            return

        client = instance.get_graphql_client()
        secondary_mappings = instance.get_secondary_lang_mappings()

        primary_mapping = instance.shopify_lang_mapping_ids.filtered("is_primary")[:1]
        primary_lang_code = (
            primary_mapping.odoo_lang_id.code if primary_mapping and primary_mapping.odoo_lang_id else (
                instance.shopify_lang_id.code if instance.shopify_lang_id else False))

        use_odoo_product = instance.import_translation_in_odoo_product
        set_sales_desc = self.env["ir.config_parameter"].sudo().get_param(
            "shopify_ept.set_sales_description")

        valid_pairs = [(t, gid) for t, gid in template_gid_pairs if gid]
        if not valid_pairs:
            return

        _logger.info(
            "batch_export_product_translations: %d product(s), chunk_size=%d, source=%s.",
            len(valid_pairs), TRANSLATION_CHUNK_SIZE,
            "odoo_product" if use_odoo_product else "shopify_layer",
        )
        ...
        for chunk_start in range(0, len(valid_pairs), TRANSLATION_CHUNK_SIZE):
            ...
            for shopify_template, product_gid in chunk:
                odoo_tmpl = shopify_template.product_tmpl_id if use_odoo_product else None

                if use_odoo_product and odoo_tmpl:
                    if primary_lang_code:
                        odoo_tmpl_primary = odoo_tmpl.with_context(lang=primary_lang_code)
                    else:
                        odoo_tmpl_primary = odoo_tmpl
                    primary_title = odoo_tmpl_primary.name or ""
                    primary_desc = (
                        odoo_tmpl_primary.description_sale
                        if set_sales_desc
                        else odoo_tmpl_primary.description
                    ) or ""
                else:
                    if primary_lang_code:
                        shopify_primary = shopify_template.with_context(lang=primary_lang_code)
                    else:
                        shopify_primary = shopify_template
                    primary_title = shopify_primary.name or ""
                    primary_desc = shopify_primary.description or ""

                title_digest = get_digest(primary_title) if primary_title else None
                desc_digest = get_digest(primary_desc) if primary_desc else None

                for mapping in secondary_mappings:
                    odoo_lang_code = mapping.odoo_lang_id.code
                    shopify_locale = mapping.shopify_locale

                    if title_digest:
                        if use_odoo_product and odoo_tmpl:
                            translated_title = odoo_tmpl.with_context(lang=odoo_lang_code).name or ""
                        else:
                            translated_title = shopify_template.with_context(lang=odoo_lang_code).name or ""
                        if translated_title and translated_title != primary_title:
                            helper.add_target_to_push_with_digest(
                                product_gid, "title", translated_title, title_digest, shopify_locale)

                    if desc_digest:
                        if use_odoo_product and odoo_tmpl:
                            translated_desc = (
                                odoo_tmpl.with_context(lang=odoo_lang_code).description_sale
                                if set_sales_desc
                                else odoo_tmpl.with_context(lang=odoo_lang_code).description
                            ) or ""
                        else:
                            translated_desc = shopify_template.with_context(lang=odoo_lang_code).description or ""
                        if translated_desc and translated_desc != primary_desc:
                            helper.add_target_to_push_with_digest(
                                product_gid, "body_html", translated_desc, desc_digest, shopify_locale)

            results = helper.push_translations()
            _logger.info("batch_export_product_translations: chunk [%d-%d] pushed %d translation(s).",
                         chunk_start, chunk_start + len(chunk) - 1, len(results))

    def set_metafield_values_in_product_template(self, template_data, instance, shopify_template):
        """
            Set metafield values in the product template based on Shopify metafield mappings.
        """
        mappings = self.env['shopify.metafield.config.ept'].get_shopify_metafield_configs(
            instance.id, ["product.template"], 'import'
        )
        if not mappings or not shopify_template:
            return True

        template_metafields = template_data.get("metafields", [])
        if not template_metafields:
            return True

        metafield_dict = {(m["namespace"], m["key"]): m["value"] for m in template_metafields}
        vals = {}
        for mapping in mappings.filtered(lambda m: m.odoo_field_id and m.model_id.model == "product.template"):
        ...
        if vals:
            shopify_template.product_tmpl_id.write(vals)
        return True

    def set_metafield_values_in_product_variant(self, template_data, instance, shopify_product=False):
        """
            Set metafield values in the product variant based on Shopify metafield mappings.
        """
        mappings = self.env['shopify.metafield.config.ept'].get_shopify_metafield_configs(
            instance.id, ["product.product"], 'import'
        )
        if not mappings or not shopify_product:
            return True

        variant_metafields = template_data.get("metafields", [])
        if not variant_metafields:
            return True

        metafield_dict = {(m["namespace"], m["key"]): m["value"] for m in variant_metafields}
        vals = {}
        for mapping in mappings.filtered(lambda m: m.odoo_field_id and m.model_id.model == "product.product"):
            key_tuple = (mapping.namespace, mapping.key)
            value = metafield_dict.get(key_tuple)
            field_name = mapping.odoo_field_id.name
            field_type = mapping.odoo_field_id.ttype
            ...
            if value is None:
                ...
                vals[field_name] = mapping._convert_shopify_to_odoo(mapping.value_type, value, mapping.odoo_field_id)
        if vals:
            shopify_product.product_id.write(vals)
        return True
