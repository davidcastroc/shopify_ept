...
class ShopifyResPartnerEpt(models.Model):
    ...
    def shopify_create_contact_partner(self, vals, instance, queue_line):
        ...
        if not first_name and not last_name and not email:
            ...
            common_log_line_obj.create_common_log_line_ept(shopify_instance_id=instance.id, module="shopify_ept",
                                                           ...
                                                           shopify_order_data_queue_line_id=queue_line.id if self.env.context.get(
                                                               'order_data_queue') else False,
                                                           shopify_customer_data_queue_line_id=queue_line.id if self.env.context.get(
                                                               'customer_data_queue') else False,
                                                           order_ref=queue_line.shopify_order_id if self.env.context.get(
                                                               'order_data_queue') else False)
            return False
        ...
        if partner:
            ...
            # If the existing partner has no country or state, update address fields from the incoming data
            if not partner.country_id or not partner.state_id:
                address_vals = self.shopify_prepare_partner_vals(vals.get("default_address", {}), instance)
                update_vals.update({k: v for k, v in address_vals.items()
                                    if k in ('street', 'street2', 'city', 'zip', 'phone', 'state_id', 'country_id') and v})
        ...
        if email:
            ...
            if partner:
                update_vals = {"is_shopify_customer": True, "category_id": [(6, 0, tag_ids)]}
                # If the existing partner has no country or state, update address fields from the incoming data
                if not partner.country_id or not partner.state_id:
                    address_vals = self.shopify_prepare_partner_vals(vals.get("default_address", {}), instance)
                    update_vals.update({k: v for k, v in address_vals.items()
        ...
        if instance.enable_metafield_sync and instance.use_graphql_api:
            self.set_metafield_values_in_customer(vals, instance, partner)
        return partner
    ...
    def shopify_create_or_update_address(self, instance, shopify_customer_data, parent_partner, partner_type="contact"):
        ...
        if instance.enable_metafield_sync and instance.use_graphql_api:
            self.set_metafield_values_in_customer(shopify_customer_data, instance, partner)


        return partner

    def set_metafield_values_in_customer(self, customer_response, instance, customer):
        """
            This method is used to set metafield values in customer.
        """
        mappings = self.env['shopify.metafield.config.ept'].get_shopify_metafield_configs(
            instance.id, ["res.partner"], 'import'
        )
        if not mappings or not customer:
            return True

        customer_metafields = customer_response.get("metafields", []) #customer_response['metafields']
        # if not customer_metafields:
        #     return True

        metafield_dict = {(m["namespace"], m["key"]): m["value"] for m in customer_metafields}
        vals = {}
        for mapping in mappings.filtered(lambda m: m.odoo_field_id and m.model_id.model == "res.partner"):
