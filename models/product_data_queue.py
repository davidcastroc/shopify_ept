...
class ShopifyProductDataQueue(models.Model):
    ...
    def shopify_create_product_data_queue(self, instance, import_based_on='', from_date=False,
        ...
        if template_ids:
            ...
            if instance.use_graphql_api:
                product_queue_list += self.fetch_and_create_queues_from_graphql_products(instance, 'active', import_based_on, from_date, to_date, skip_existing_product)
                results = True
            else:
            ...
            if not instance.use_graphql_api and len(results) >= 250:
            ...
            if is_import_draft_product:
                if instance.use_graphql_api:
                    product_queue_list += self.fetch_and_create_queues_from_graphql_products(instance, 'draft', import_based_on, from_date, to_date, skip_existing_product)
                    results = True
                else:
        ...
        if not results:
            ...
            return False
        if product_queue_list:
            product_queue_cron = self.sudo().env.ref("shopify_ept.process_shopify_product_queue")
            if not product_queue_cron.active:
                _logger.info("Active the Order data process queue cron job")
                product_queue_cron.write(
    ...
    def fetch_and_create_queues_from_graphql_products(self, instance, status, import_based_on, from_date, to_date, skip_existing_product, fields=None):
        """
        Fetch products from Shopify using GraphQL API via ProductQueryHelper.
        Handles pagination, rate limits, and creates product queues in batches.
        """
        client = instance.get_graphql_client()
        product_obj = shopify_graphql.ProductQueryHelper(client)
        product_queue_list = []
        has_next_page = True
        extra_conn = None
        query_filter = product_obj.build_date_filter(status, import_based_on, from_date, to_date)        
        while has_next_page:
            batch_products, has_next_page, end_cursor = product_obj.get_products(query_filter=query_filter,
                                                                                 no_pagination=True,
                                                                                 extra_connection_args=extra_conn,
                                                                                 convert_to_rest=True,
                                                                                 fetch_metafields= True if instance.enable_metafield_sync else False,
                                                                                 )
            if end_cursor:
                extra_conn = f'after:"{end_cursor}"'
            if batch_products:
                product_queue_list += self.create_product_queues(instance, batch_products, skip_existing_product)
            _logger.info("Processed batch of %d products, next cursor: %s", len(batch_products), end_cursor)
        return product_queue_list
    ...
    def import_products_by_remote_ids(self, template_ids, instance):
        ...
        if len(template_ids.split(",")) <= 100:
            ...
            if instance.use_graphql_api:
                client = instance.get_graphql_client()
                results = shopify_graphql.ProductQueryHelper(client).get_product(template_ids, convert_to_rest=True, 
                fetch_metafields= True if instance.enable_metafield_sync else False)
    ...
    def shopify_create_product_data_queue_line(self, result, instance, product_data_queue):
        ...
        existing_product_data = product_data_queue_line_obj.search(
            [('product_data_id', '=', result.get('id')), ('shopify_instance_id', '=', instance.id),
             ('state', 'in', ['draft', 'failed']), ('product_data_queue_id.is_action_require', '=', False)], limit=1)
    ...
    def import_product_cron_action(self, ctx=False):
        ...
        if isinstance(ctx, dict):
            ...
            ctx.update({'is_import_product_cron': True})
            self.with_context(ctx).shopify_create_product_data_queue(instance, import_products_based_on_date, from_date, to_date,
