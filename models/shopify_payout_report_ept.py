...
class ShopifyPaymentReportEpt(models.Model):
    ...
    def get_payout_report(self, start_date, end_date, instance):
        ...
        payout_reports = []
        if getattr(instance, 'use_graphql_api', False):
            # Use GraphQL API
            try:
                ...
                filters = [f"issued_at:>='{start_date}'", f"issued_at:<='{end_date}'", f"status:paid"]
                payout_reports = shopify_graphql.PayoutQueryHelper(client).get_payouts(first=250, query_filter=filters)
            except Exception as error:
                message = f"GraphQL: Error importing payout records: {error}"
    ...
    def create_payout_transaction_lines(self, payout_data):
        ...
        if getattr(instance, 'use_graphql_api', False):
            client = instance.get_graphql_client()
            transaction_all = shopify_graphql.TransactionQueryHelper(client).get_transactions(
                payout_id=self.payout_reference_id,rest_format=True)
        else:
        ...
        for transaction in transaction_all:
            ...
            transaction_vals = self.prepare_transaction_vals(transaction_data, instance)
            shopify_payout_report_line_obj.create(transaction_vals)

        # Create fees line
        summary = payout_data.get('summary', {})
        fees_amount = float(summary.get('charges_fee_amount', 0.0)) + float(
            summary.get('refunds_fee_amount', 0.0)) + float(
            summary.get('adjustments_fee_amount', 0.0))
    ...
    def generate_bank_statement(self):
        ...
        if not journal:
            return False
        self.create_bank_statement_lines_for_payout_report(journal)
    ...
    def create_bank_statement_lines_for_payout_report(self, journal):
        """
        ...
        for transaction in transaction_ids:
            ...
            if transaction.amount:
                ...
                transaction_config_line = self.instance_id.transaction_line_ids.filtered(
                    lambda l: l.transaction_type == transaction.transaction_type)
                counter_part_account_id = transaction_config_line.account_id
                bank_line_vals = {
                ...
                st_line = bank_statement_line_obj.create(bank_line_vals)
                transaction.is_remaining_statement = False

                if transaction.transaction_type not in ['charge', 'refund', 'payment_refund']:
                    if self.instance_id.shopify_is_use_analytic_account and transaction_config_line and transaction_config_line.analytic_account_id:
                        analytic_account_id = transaction_config_line.analytic_account_id.id
                        counterpart_move_line = st_line.move_id.line_ids.filtered(
                            lambda l: l.account_id.id == counter_part_account_id.id)
                        if counterpart_move_line:
                            counterpart_move_line.with_context(check_move_validity=False).write(
    ...
    def check_journal_and_currency(self):
        ...
        @author: Maulik Barad on Date 02-Dec-2020.
        """
        journal = self.instance_id.get_payout_journal(self.currency_id)
        if not journal:
    ...
    def get_paid_move_line_amount(self, statement_line, paid_invoices):
        ...
        if statement_line.refund_invoice_id:
            ...
            if not paid_move_lines:
                paid_move_lines = paid_invoices.line_ids.filtered(
        ...
        for moveline in paid_move_lines:
            ...
            if moveline.amount_currency:
                currency, amount_currency = self.convert_move_amount_currency(statement_line, moveline, amount,
                                                                              statement_line.date)
                if currency:
        ...
        if round(statement_line.amount, 10) == round(move_line_total_amount, 10) and (
            ...
            try:
                if move_line_data:
                    ...
                    move_line_id = [move_line['id'] for move_line in move_line_data if move_line.get('id')]
                    self.shopify_reconcile_bank_statement_line_ept(statement_line.id, move_line_id)
                # for payment_line in paid_move_lines:
                if paid_move_lines:
                    self.shopify_reconcile_bank_statement_line_ept(statement_line.id, paid_move_lines.ids)
            except Exception as error:
    ...
    def shopify_reconcile_bank_statement_line_ept(self, statement_line_id, move_line_id):
        ...
        """
        statement_line_id = self.env['account.bank.statement.line'].browse(statement_line_id)
        statement_line_id.with_user(SUPERUSER_ID).set_line_bank_statement_line(move_line_id)
    ...
    def shopify_reconcile_other_bank_statement_line_ept(self, statement_line_id, move_line_id):
        ...
        account_id = transaction_account_line.account_id
        statement_line_id.set_account_bank_statement_line(statement_line_id.line_ids[-1].id, account_id.id)

        # Set analytic account if configured
        edit_data = {}
        analytic_account = transaction_account_line.analytic_account_id if \
            self.instance_id.shopify_is_use_analytic_account else False
        if analytic_account:
            edit_data.update({'analytic_distribution': {analytic_account.id: 100}})
        if edit_data:
            statement_line_id.edit_reconcile_line(statement_line_id.line_ids[-1].id, edit_data)
