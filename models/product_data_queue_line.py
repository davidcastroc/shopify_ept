...
class ShopifyProductDataQueueLineEpt(models.Model):
    ...
    def auto_import_product_queue_line_data(self):
        ...
        product_data_queue_list = self.env.cr.fetchall()
    ...
    def process_product_queue_line_data(self):
        ...
        if queue_id:
            ...
            if shopify_instance.active:
                _logger.info("Instance %s is active.", shopify_instance.name)
                ...
                for product_queue_line in self:
                    shopify_template = shopify_product_template_obj.shopify_sync_products(product_queue_line,
                                                                       False,
                                                                       shopify_instance)
                    queue_id.is_process_queue = True
                    self.env.cr.commit()
                    if shopify_template and shopify_instance.is_translations_needed():
                        translation_pairs.append((shopify_template, shopify_template.shopify_tmpl_id))
                # Batch-import ALL translations in the minimum number of API calls
                if translation_pairs:
                    shopify_product_template_obj.batch_import_product_translations(shopify_instance, translation_pairs)
                    self.env.cr.commit()
    ...
    def query_find_queue_line_for_import_image(self):
        ...
        params = ('done', 'pending')
        self.env.cr.execute(query, params)
        product_queue_lines = self.env.cr.fetchall()
