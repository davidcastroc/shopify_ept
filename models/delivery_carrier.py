...
class DeliveryCarrier(models.Model):
    ...
    def shopify_search_create_delivery_carrier(self, line, instance):
        ...
        carrier.invalidate_model(['write_uid'])
        return carrier
    ...
    def search_carrier_for_webhook_fulfillment(self, instance, fulfillment_data):
        ...
        if not carrier_company:
            ...
            if tracking_info:
                carrier_company = tracking_info[0].get('company')
                fulfillment_data['tracking_number'] = tracking_info[0].get('number')
        if carrier_company:
            carrier = self.search([('shopify_tracking_company', '=', carrier_company), ('company_id', 'in',
                                                                    [instance.shopify_company_id.id, False])], limit=1)
            if carrier:
                return carrier
        carrier_name = fulfillment_data.get('service')
        if isinstance(carrier_name, dict):
            carrier_name = carrier_name.get('handle')
        carrier = False
        if carrier_name:
            carrier = self.search([('name', '=', carrier_name),
                                   ('company_id', 'in', [instance.shopify_company_id.id, False])], limit=1)
            if not carrier:
                carrier = self.create(
                    {'name': carrier_name, 'shopify_code': carrier_name, 'shopify_source': carrier_name,
                     'product_id': instance.shipping_product_id.id, 'company_id': instance.shopify_company_id.id})
        return carrier
