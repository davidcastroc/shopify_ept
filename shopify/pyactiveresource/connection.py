...
# Thread-local storage so each thread (each Odoo worker handling one instance's request)
_thread_local = threading.local()


def set_suppress_connection_info_logs(value):
    """
    Set the INFO log skip flag for the CURRENT thread only.
    Called in connect_in_shopify() right before Shopify REST calls are made,
    so it reflects the skip_api_logs setting from Shopify > Configuration > Settings.
    """
    _thread_local.suppress_info = bool(value)
    
def _is_suppressed():
    """Return True if INFO log suppression is active on the current thread."""
    return getattr(_thread_local, 'suppress_info', False)
